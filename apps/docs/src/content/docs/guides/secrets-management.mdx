...
RAPID automatically forwards your SSH agent to dev containers, enabling secure git operations (push, pull, commit signing) without exposing private keys.

### How It Works

**SSH Agent Forwarding** (automatically configured):
...
When you run `rapid dev`, the SSH agent is automatically forwarded:

```bash
# Inside the container, git operations work seamlessly
git push origin main
git commit -S -m "Signed commit"
```

### Lima VM Integration (macOS)

When using Lima VM for local development (`rapid dev --local` on macOS), SSH agent forwarding is handled automatically:

```bash
# RAPID forwards SSH agent to Lima VM
rapid dev --local

# SSH operations work inside the VM
git clone git@github.com:user/repo.git
```

### Manual Configuration

If you need to customize SSH forwarding in your `devcontainer.json`:
...
For HTTPS-based authentication, use GitHub CLI inside the container:

```bash
# Inside container
gh auth login

# Now HTTPS operations work
git clone https://github.com/user/repo.git
```

GitHub CLI uses short-lived tokens with scoped permissions.
