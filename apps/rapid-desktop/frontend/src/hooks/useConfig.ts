...
export function useConfig() {
  ...
  const { data: configData, loading, error, execute: loadConfig } = useAsyncOperation<Record<string, unknown>>(
    async () => {
  ...
  const config: RapidConfig | null = configData
    ? {
        ...
        project: {
          ...
          description: (configData.project as Record<string, unknown>)?.description as string | undefined,
        },
        sandbox: {
          enabled: ((configData.sandbox as Record<string, unknown>)?.enabled as boolean) ?? false,
          preset: ((configData.sandbox as Record<string, unknown>)?.preset as 'strict' | 'balanced' | 'permissive' | 'none') ?? 'balanced',
          allowNetwork: ((configData.sandbox as Record<string, unknown>)?.allowNetwork as boolean) ?? false,
        },
        secrets: configData.secrets as { provider: string } | undefined,
        personas: (configData.personas as Record<string, { systemPrompt: string; capabilities: string[] }>) || undefined,
        mcp: (configData.mcp as { servers?: Record<string, { command: string; args?: string[] }> }) || undefined,
      }
    : null
  ...
  const handleSave = useCallback(
    async (updatedConfig: RapidConfig) => {
      setSaving(true)
      setSaveError(null)
      ...
      try {
        ...
        const appError = err instanceof AppError ? err : new AppError(String(err), 'UNKNOWN_ERROR', 'error', false)
