  ...
  test('should display events list or empty state', async () => {
    ...
    if (await firstLink.isVisible()) {
      ...
      const messageCard = page
        .locator('.card')
        .filter({ hasText: /completion|discovery|coordination|error/ });
    ...
    if (await firstLink.isVisible()) {
      ...
      if (await completionFilter.isVisible()) {
        ...
        const activeFilter = page.locator(
          'button:has-text("completion").badge-success, button:has-text("completion").bg-green'
        );
  ...
  test('should expand event details on click', async () => {
    ...
    if (await firstLink.isVisible()) {
      ...
      const eventCard = page
        .locator('.card')
        .filter({ hasText: /completion|discovery|coordination/ })
        .first();
  ...
  test('should display event metadata', async () => {
    ...
    if (await firstLink.isVisible()) {
      ...
      if (await eventCard.isVisible()) {
        ...
        const hasAgentInfo = await eventCard
          .locator('text=/orchestrator|worker|designer/')
          .isVisible();
